...
export default function SelectedWork() {
  const featuredLabels = clients.slice(0, 3);

  return (
    <SectionShell id="featured-labels" className="py-32 relative z-10 border-t border-[var(--purple-wash)]">
      <div className="max-w-[1280px] mx-auto px-6 lg:px-12">
        ...
        <div className="flex flex-col gap-24 md:gap-32">
          {featuredLabels.map((label, index) => (
            <Reveal key={label.name} className="group relative grid grid-cols-1 md:grid-cols-12 items-center cursor-pointer">
              ...
              <div className="md:col-start-1 md:col-span-8 md:row-start-1 relative aspect-[4/3] bg-[var(--purple-soft)] rounded-xl overflow-hidden soft-lift z-10 border border-white/50">
                 ...
                 <div className="absolute inset-0 flex items-center justify-center p-12">
                   <img
                     src={label.logo}
                     alt={label.name}
                     className="max-h-28 md:max-h-36 max-w-[70%] object-contain mix-blend-multiply opacity-75 group-hover:opacity-100 transition-opacity duration-300"
                   />
                 </div>
              ...
              <div className="md:col-start-7 md:col-span-6 md:row-start-1 flex flex-col items-start z-20 mt-8 md:mt-0">
                <div className="bg-white/80 backdrop-blur-lg border border-white/60 p-8 md:p-12 rounded-2xl soft-lift w-full">
                  ...
                  <p className="text-lg font-light text-[var(--secondary)] mb-8 leading-relaxed">
                    A featured label in the DSocial brand ecosystem.
                  </p>
                  
                  <div className="flex flex-wrap gap-2">
                    {["Strategy", "Content", "Social"].map((discipline) => (
                      <span 
                        key={discipline} 
